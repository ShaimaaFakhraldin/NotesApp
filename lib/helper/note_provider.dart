...
import 'package:shaimaa_notes/helper/database_helper.dart';
...
class NoteProvider with ChangeNotifier {
  ...
  Future addOrUpdateNote(int id, String title, String content, String imagePath,
      EditMode editMode ,int status ) async {
    final note = Note(id, title, content, imagePath ,status);
  ...
  Future getNotes() async {
    ...
    _items = notesList
        .map(
          (item) => Note(
              item['id'], item['title'], item['content'], item['imagePath'] ,item['status'] ),
        )
        .toList();
